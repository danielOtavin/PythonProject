...
from users import User, ADMIN, TEST, ERROR
...
class TestUserPositive:
    ...
    def test_create_user_without_token(self, random_user, user_api):
        response = user_api.create_raw(random_user, token='')
        assert response.status_code == 403

    def test_create_user_without_login(self, random_user, user_api, admin_token):
        user_data = User(login='', password=User.random_user().password)
        response = user_api.create_raw(user_data, admin_token)
        assert response.status_code == 400

    def test_create_user_without_password(self, random_user, user_api, admin_token):
        user_data = User(login=User.random_user().login, password='')
        response = user_api.create_raw(user_data, admin_token)
        assert response.status_code == 400
    ...
    def test_create_user_with_equal_login_and_password(self, random_user, user_api, admin_token):
        user_data = User(login=User.random_user().login, password=User.random_user().login)
        response = user_api.create_raw(user_data, admin_token)
        assert response.status_code == 400
    ...
    def test_create_user_with_password_digits(self, random_user, user_api, admin_token):
        user_data = User(login=User.random_user().login, password='1234567890')
        response = user_api.create_raw(user_data, admin_token)
        assert response.status_code == 400

    def test_create_user_with_password_letters(self, random_user, user_api, admin_token):
        user_data = User(login=User.random_user().login, password='abcdefgh')
        response = user_api.create_raw(user_data, admin_token)
        assert response.status_code == 400
